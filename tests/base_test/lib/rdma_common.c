...
void rdma_set_debug(bool enabled) { rdma_debug_enabled = enabled; }

void rdma_log(const char *fmt, ...) {
  if (!rdma_debug_enabled)
    return;
...
int rdma_init_context(struct rdma_context *ctx,
                      const struct rdma_config *config) {
  if (!ctx || !config) {
  ...
  if (ctx->dev_index >= num_devices || !dev_list[ctx->dev_index]) {
    fprintf(stderr,
            "[ERROR] Device index %d not available (found %d devices)\n",
  ...
  // Allocate buffer
  if (rdma_alloc_buffer(&ctx->buffer, ctx->buffer_size, config->use_hugetlb) <
      0) {
    goto err_dealloc_pd;
  }
  ...
  struct ibv_qp_init_attr qp_init_attr = {
      .send_cq = ctx->send_cq,
      .recv_cq = ctx->recv_cq,
      .cap = {.max_send_wr = config->max_send_wr,
...
int rdma_connect_qp(struct ibv_qp *qp, uint32_t dest_qp_num,
                    uint32_t dest_gid_ipv4) {
  if (rdma_qp_to_init(qp) < 0)
    return -1;
  if (rdma_qp_to_rtr(qp, dest_qp_num, dest_gid_ipv4) < 0)
    return -1;
  if (rdma_qp_to_rts(qp) < 0)
    return -1;
